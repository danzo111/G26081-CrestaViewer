...
class NetworkViewerApp {
  ...
  async init() {
    try {
      ...
      // Load main network data
      const networkData = await dataLoader.loadNetworkData('network.json');

      // Load dummy pipes from network_with_dummies.json and merge them
      try {
        const dummyResponse = await fetch('network_with_dummies.json', {
          method: 'GET',
        ...
        if (dummyResponse.ok) {
          ...
          // Extract dummy pipes (IDs starting with DUMMY_PIPE)
          const dummyPipes = (dummyData.pipes || []).filter(p => 
            p.id && p.id.startsWith('DUMMY_PIPE')
          );

          // Add dummy manholes to the manhole list so pipes can reference them
          const dummyManholes = (dummyData.manholes || []).filter(m => 
            m.id && m.id.startsWith('DUMMY')
          );

          // Merge dummy manholes into the main manhole array
          if (dummyManholes.length > 0) {
            networkData.manholes.push(...dummyManholes);
          }

          // Merge dummy pipes into the main pipes array
          if (dummyPipes.length > 0) {
            networkData.pipes.push(...dummyPipes);
            console.log(`Added ${dummyPipes.length} dummy pipes to the network`);
          }

          // Update metadata to reflect the merged data
          if (!networkData.metadata) networkData.metadata = {};
          networkData.metadata.dummy_pipes_count = dummyPipes.length;
          networkData.metadata.dummy_manholes_count = dummyManholes.length;
          networkData.metadata.total_pipes = networkData.pipes.length;
          networkData.metadata.total_manholes = networkData.manholes.length;
        }
      } catch (dummyErr) {
        console.warn('Could not load dummy pipe data:', dummyErr.message);
      }
